...
Dedup flow: `music dedup search` writes `Deduplication` groups → `music dedup review` lets the model decide the doubtful entries → `music dedup process` merges them via `MergeService`. See "Deduplication" below.

### Deduplication: recall, then score, then ask

`src/services/deduplication/` decides whether two song documents are the same recording, in three passes that are three CLI subcommands. The bias is stated everywhere: a wrong merge deletes a document and re-points an album, so **better no merge than a wrong merge**.

- **`search` recalls, then scores in code.** `SearchDuplicateCandidatesQuery` is deliberately loose and nothing it returns is a verdict. Three routes in, any one enough: fuzzy title *and* artist over several subfields, the exact whole-string title *and* artist, or a shared ISRC; the album is never a clause, and candidates on the same album document are boosted. `duplicate-score.util.ts` is the verdict: pure functions over the two populated documents, unit-tested, that parse version qualifiers out of titles (`parseTitle` keeps `(Part 2)` in the name, pulls `(Live)` and `- Remastered 2009` out as markers, and `(feat. X)` out as a credit), split an album from its edition (`parseAlbum`), and place the pair in a tier. `reject` on any hard contradiction: a recording marker on one side only, durations more than 15s apart, titles that differ by a number, a different track or disc number on the same record in the same edition, an artist below 0.7 (`Spice` vs `Spice Girls` scores 0.67 and must never get further), or a different album, which is a different song by policy. `auto` only when title, artist and album all clear 0.9 with nothing capping it, when the ISRC is shared on the same record, or when the pair sits in the same slot (same album, disc and track number) with a title above 0.8. Everything in between is `review`. Rejected pairs are never stored; `auto` and `review` entries are written with their signals and reasons.
- **Symbols are part of a name, so exact comes before tokenised.** `normalizeForMatch` strips everything but letters and digits, and for some names that is the whole name: M.I.A.'s `/\/\/\ Y /\` tokenises to `y`, `bbno$` to `bbno`, and `M.I.A.` would collide with `Mia`. `compareNames` in the scorer therefore tries a whole-string, case-folded, symbol-preserving equality first, and refuses the tokenised comparison altogether when either side tokenises to under two characters. The index mirrors this: every one of `title`, `artist` and `album` carries an `exact` keyword subfield (`exact_normalizer`, lowercase and trim only) that the recall query hits as its own route. The usual full-text approach alone does not work here, and every layer is written knowing it.
- **The triangle is the identity.** A song hangs off an artist document and an album document, and the scorer reads both ids: the same album document is the same record whatever the two titles spell (`albumId`), the same artist document the same artist. On the same record in the same edition the track and disc numbers are the slot, and two different slots are two different songs however alike the titles read; across editions a track number means nothing, since a deluxe repackaging renumbers. Duration is a soft signal by design: a local mp3 and the same recording on a streaming service can differ by a few seconds, so it only caps a tier below 15s and never decides on its own.
- **`review` is the AI tier.** `DuplicateVerdictRequest` (hosted by `EnrichAgent`, Plain shape, one pair per request) hands the model both entries plus the scorer's signals and asks for `same`/`confidence`/`reason`. The verdict is recorded on the entry as `decision` with `decidedBy: ai`; a `same` below 0.7 confidence is written as `different`. The prompt is told to prefer `different` when the evidence is thin, for the same reason as above.
- **`process` merges only certainty.** `auto` entries and entries decided `same`; `different` entries are left; a group stays `pending` while any entry still waits. An entry with no `tier` predates the tiers and is read as `auto`, since the old search only wrote what it considered certain.
- **The cascade is guarded.** `MergeService` merges artists or albums only when `isSameEntityName` agrees on the names; otherwise the duplicate song is folded into the primary and its artist and album are left exactly as they were. Merging the song is one document lost if wrong; merging the artist re-points a discography.
- **ISRC is the one identity key that survives spelling.** `SongSource.isrc` is filled by the Qobuz builder (always) and the Spotify builder (rarely, since Development Mode apps no longer receive `external_ids`). Two sources sharing one are the same recording; the scorer treats it as proof of the recording and leaves only the album open.
- **The recall query has a clause budget.** Lucene rewrites every fuzzy term into its neighbours per field and refuses a query past `max_clause_count` (1,024); a twenty-token classical title fuzzy over five subfields is five thousand clauses, and that is what failed every `/songs/_search` once the recreated index carried every subfield. `SearchDuplicateCandidatesQuery` therefore puts fuzziness on `icu` and `identity` only, caps it at ten expansions with the first letter fixed, drops it altogether past twelve tokens, and matches the other three subfields on exact tokens. A recall that fails is rethrown and counted as an error by the run, not read as "no duplicates".
- **Several readings of every name, so no single plugin is load-bearing.** `SongIndices` declares, on each of `title`, `artist` and `album`: `identity` (standard tokenizer, `asciifolding`, `&` read as `and`, no plugin), `icu` (whitespace tokenizer, `icu_transform` `Any-Latin; Latin-ASCII` then `icu_folding`, so Cyrillic, Hangul, Han and kana all become Latin and `Beyoncé` is `beyonce`), the existing `pinyin` and `romaji` readings, and `exact`. The `icu` analyzer is whitespace-tokenised on purpose: the ICU tokenizer would drop the `$`, the dots and the whole of `/\/\/\`. The recall query lists all of them; on an index created before a subfield existed that subfield matches nothing and the others carry the query. `opensearch prune` then `create` and `index` bring the new ones in; the cluster needs `analysis-icu` installed for `create` to accept the mapping.
- The importers' own existing-song check (`findDuplicatesSongs`, the `_score >= 100` contract) is unchanged, except that its terms are no longer wrapped in `"""`: those quotes reached the analyzers as text and silenced the `keyword` and `normalizer` subfields, so matching ran on the pinyin and romaji fields alone.
...
- **The three catalogs form a ladder: Qobuz → Spotify → YouTube.** Qobuz streams lossless, Spotify 320 kbps Ogg, YouTube 256 kbps AAC, and Qobuz and Spotify are licensed catalogs with an artist id behind every hit where YouTube's artist is parsed off an upload title. The chat prompt, every tool description and every dead-end reply are worded around that order: an empty Qobuz answer points at Spotify, an empty Spotify answer points at YouTube, and an empty YouTube answer ends the search. Change the order in one place and the model gets contradictory instructions from the others. The Spotify tools (`tools/handler/spotify/`) mirror the Qobuz pair — `spotify_search_artist` / `spotify_search_music` / `spotify_start_playback` — and `SpotifyService.searchArtistCatalog` is the artist-locked lookup behind them: the name is resolved to a Spotify artist id and every hit is verified against it by `trackBelongsToArtist`, because Spotify's `artist:` filter is a text match over the credits and "Spice" still pulls in "Spice Girls". Like the Qobuz and YouTube pairs, nothing is written to Mongo.
...
never swapped for its Qobuz equivalent. YouTube *is* the last rung of that pass, though — see below.
...
`src/services/negentropy/` scans the MPD queue **ahead of the playhead** every 20s, finds entries playing from a low quality local file, and swaps them for a streaming provider's rendition of the same recording — attaching that provider's `SongSource` to the song document on the way.
...
- **Providers are a ladder, and the file decides where it starts.** `UPGRADE_PROVIDERS` in `quality.util.ts` is `qobuz → spotify → youtube`, which is the quality order (lossless, 320 kbps Ogg, 256 kbps AAC); the pass stops at the first match. `providerBeatsFile` prunes the rungs that would not be an upgrade before any call is made: a 320 kbps mp3 only ever costs a Qobuz lookup, a 256 kbps one is worth Spotify but not YouTube, a lossy file with no bitrate gets Spotify (an mp3 tops out at 320) but not YouTube, and a file with no technical info at all gets Qobuz only — it could be a FLAC. The provider figures there are the same ones the three `SongSource` builders write into `technical_info`, so a swap this pass makes is also the source `PlayMusicHandler.getBestSource` picks afterwards. Keep them in step.
- **A provider that cannot be played is not asked.** `isProviderAvailable` drops a rung when it is missing from `ACTIVE_SOURCE_TYPES` or its proxy variable (`QOBUZ_STREAM_PROXY_SERVER`, `SPOTIFY_PROXY_AUDIO`, `YOUTUBE_PROXY_AUDIO`) is unset. A candidate left with no rung is counted `skipped` and **not** written to the ledger, so it is picked up again once the provider is configured.
- **`negentropy_job` is the anti-spam ledger.** One document per song, unique on `songId`, written for `upgraded`, `no_match` **and** `failed` alike, with `provider`, `sourceId` and `providersTried`. Without it a 20s cycle would re-query the providers about an unchanged queue. Delete a document to force a re-check; `no_match` is otherwise permanent — including one recorded while a provider was down, which is the reason to delete it. A **rate limit is not recorded**: a Spotify 429 or a YouTube quota error (`isSpotifyRateLimited`, `isYoutubeQuotaError`) marks the song `deferred` and leaves no document, so the next pass climbs the ladder again.
- **Spotify in Development Mode pages at 10 and throttles hard.** Every listing call above `limit=10` answers `400 Invalid limit`, and a dozen calls in quick succession answer `429 QUOTA_EXCEEDED`. `SpotifyService` pages lazily with `SEARCH_PAGE_CEILING = 10` and stops as soon as it has enough; do not raise the ceiling until the app passes Spotify's quota review.
- Candidates are decided by `lowQualityReason`: lossy format, or sub-CD-quality technical info, or no technical info at all. Only `file` sources qualify.
- Match threshold is **0.85** on every provider's `findTrack`, higher than the CLI default — this replaces something already queued. YouTube additionally has to clear a title floor and an artist floor, because its total can be carried by the artist alone and right-artist-wrong-song is the one swap worse than none.
...
- A song that already carries a streaming source **that beats the file** (`existingSourceBeatsFile`, ladder order) is swapped with no lookup and no job record — the uri check skips it on the next pass. A 320 kbps mp3 sitting next to a spotify source stays on the file.
- **Artwork rides along with the source.** Every provider's match carries a cover (Qobuz's album image, Spotify's album images, YouTube's best thumbnail), and it is written to the song's album document when that album has none (`MusicDbService.setAlbumImageIfMissing`, never an overwrite). This is not cosmetic: MPD can read a picture out of a local file but not out of a proxied stream, so a swap would otherwise leave the track with no artwork. MPD has **no artwork tag** — `addtagid` rejects Artwork/Picture/AlbumArt with `Unknown tag type` — the album document is the only place the cover can live. The `reused` path does not do this, since no source is being added and the track payload is not in hand.
- Gated by `NEGENTROPY_ENABLED` (on when unset) and by `IS_CLI`, same as the playlog poller. `npm run cli -- negentropy run [--dry-run] [--limit n]` runs one pass by hand; `--limit` counts songs, each of which may take up to three provider calls.
