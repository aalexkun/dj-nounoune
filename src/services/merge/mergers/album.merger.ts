...
export class AlbumMerger implements Merger<AlbumDocument> {
  ...
  merge(primaryEntity: AlbumDocument, duplicateEntity: AlbumDocument): AlbumDocument {
    this.logger.log(
    ...
    if (duplicateEntity.tracks && duplicateEntity.tracks.length > 0) {
      ...
      const existingTrackIds = new Set(primaryEntity.tracks.map((t) => (t as unknown as Types.ObjectId).toString()));
    ...
    primaryEntity.title = resolveFieldValue('title', primaryEntity.title, duplicateEntity.title, primarySources, duplicateSources);
    primaryEntity.release_year = resolveFieldValue(
      'release_year',
      primaryEntity.release_year,
      duplicateEntity.release_year,
      primarySources,
      duplicateSources,
    );
    primaryEntity.genre = resolveFieldValue('genre', primaryEntity.genre, duplicateEntity.genre, primarySources, duplicateSources);
    primaryEntity.record_label = resolveFieldValue(
      'record_label',
      primaryEntity.record_label,
      duplicateEntity.record_label,
      primarySources,
      duplicateSources,
    );
    primaryEntity.catalogue_number = resolveFieldValue(
      'catalogue_number',
      primaryEntity.catalogue_number,
      duplicateEntity.catalogue_number,
      primarySources,
      duplicateSources,
    );
    primaryEntity.subtitle = resolveFieldValue('subtitle', primaryEntity.subtitle, duplicateEntity.subtitle, primarySources, duplicateSources);
    primaryEntity.description = resolveFieldValue(
      'description',
      primaryEntity.description,
      duplicateEntity.description,
      primarySources,
      duplicateSources,
    );
    primaryEntity.release_date_original = resolveFieldValue(
      'release_date_original',
      primaryEntity.release_date_original,
      duplicateEntity.release_date_original,
      primarySources,
      duplicateSources,
    );
    ...
    primaryEntity.total_duration = resolveFieldValue(
      'total_duration',
      primaryEntity.total_duration,
      duplicateEntity.total_duration,
      primarySources,
      duplicateSources,
    );
    primaryEntity.album_gain = resolveFieldValue(
      'album_gain',
      primaryEntity.album_gain,
      duplicateEntity.album_gain,
      primarySources,
      duplicateSources,
    );
    primaryEntity.rating = resolveFieldValue('rating', primaryEntity.rating, duplicateEntity.rating, primarySources, duplicateSources);
    primaryEntity.track_count = resolveFieldValue(
      'track_count',
      primaryEntity.track_count,
      duplicateEntity.track_count,
      primarySources,
      duplicateSources,
    );
    primaryEntity.is_complete = resolveFieldValue(
      'is_complete',
      primaryEntity.is_complete,
      duplicateEntity.is_complete,
      primarySources,
      duplicateSources,
    );
    ...
    primaryEntity.image = this.mergeImage(primaryEntity.image, duplicateEntity.image, primarySources, duplicateSources);
  ...
  private mergeImage(
    ...
    const countNonEmpty = (img: NonNullable<AlbumDocument['image']>): number => {
      return [img.small, img.thumbnail, img.large, img.back].filter(Boolean).length;
    };
