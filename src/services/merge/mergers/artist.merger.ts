...
export class ArtistMerger implements Merger<ArtistDocument> {
  ...
  merge(primaryEntity: ArtistDocument, duplicateEntity: ArtistDocument): ArtistDocument {
    this.logger.log(
    ...
    if (duplicateEntity.albums && duplicateEntity.albums.length > 0) {
      ...
      const existingAlbumIds = new Set(primaryEntity.albums.map((a) => a.toString()));
    ...
    primaryEntity.artist = resolveFieldValue('artist', primaryEntity.artist, duplicateEntity.artist, primarySources, duplicateSources);
    primaryEntity.primary_genres = resolveFieldValue(
      'primary_genres',
      primaryEntity.primary_genres,
      duplicateEntity.primary_genres,
      primarySources,
      duplicateSources,
    );
    primaryEntity.short_intro = resolveFieldValue(
      'short_intro',
      primaryEntity.short_intro,
      duplicateEntity.short_intro,
      primarySources,
      duplicateSources,
    );
    primaryEntity.biography = resolveFieldValue('biography', primaryEntity.biography, duplicateEntity.biography, primarySources, duplicateSources);
