...
export class SongMerger implements Merger<SongDocument> {
  ...
  async merge(primaryEntity: SongDocument, duplicateEntity: SongDocument): Promise<SongDocument> {
    this.logger.log(
    ...
    if (duplicateEntity.source && duplicateEntity.source.length > 0) {
      ...
      for (const newSource of duplicateEntity.source) {
        const sourceExists = primaryEntity.source.some((s) => s.name === newSource.name && s.sourceId === newSource.sourceId);
    ...
    primaryEntity.year = resolveFieldValue('year', primaryEntity.year, duplicateEntity.year, primarySources, duplicateSources);
    primaryEntity.track_number = resolveFieldValue(
      'track_number',
      primaryEntity.track_number,
      duplicateEntity.track_number,
      primarySources,
      duplicateSources,
    );
    primaryEntity.disc_number = resolveFieldValue(
      'disc_number',
      primaryEntity.disc_number,
      duplicateEntity.disc_number,
      primarySources,
      duplicateSources,
    );
    primaryEntity.composer = resolveFieldValue('composer', primaryEntity.composer, duplicateEntity.composer, primarySources, duplicateSources);
    primaryEntity.album_artist = resolveFieldValue(
      'album_artist',
      primaryEntity.album_artist,
      duplicateEntity.album_artist,
      primarySources,
      duplicateSources,
    );
    primaryEntity.category = resolveFieldValue('category', primaryEntity.category, duplicateEntity.category, primarySources, duplicateSources);
    ...
    primaryEntity.lyric_semantic = resolveFieldValue(
      'lyric_semantic',
      primaryEntity.lyric_semantic,
      duplicateEntity.lyric_semantic,
      primarySources,
      duplicateSources,
    );
